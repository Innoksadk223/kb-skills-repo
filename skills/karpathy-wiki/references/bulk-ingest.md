...
## Parallel workflow (5+ independent sources — use subagents)
...
2. **Split sources into groups** of 2-4 files each. Each group gets a read-only subagent task.
...
> ⚠️ **Subagent file-mutation hazard (critical pitfall):** subagent workers may share or fork from the same filesystem context. A subagent that writes to `index.md` or creates wiki pages can:
